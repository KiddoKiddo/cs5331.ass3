...
Typically, the scanner will consist of 4 phases. The communication between these phases \underline{{\em must} be via files in {\tt JSON} format}.
...
This assignment accounts for 50\% (50 points) of your final grade. The following scheme will be applied when we grade your scanner.
	...
	\item How many bugs does your scanner find from our hidden benchmark? [{\bf 20 points}]
...
\subsection{CSRF} 
\paragraph{Scope} Carry out a sensitive operation on behalf of other users by exploiting the lack of proper csrf token/mechanism. Sensitive operation is the operation operated on sensitive data and it is left to you to define which data is sensitive. Note that normal form can be unprotected by CSRF if there is nothing sensitive that can be achieved by submitting that form.
